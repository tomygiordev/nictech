...
interface RestockItem {
  ...
  qty: number;
  unitCost: number;       // costo de compra (opcional)
  currentStock: number;
}

interface RestockSnapshot {
  orderNumber: string;
  date: string;
  items: RestockItem[];
  supplier: string;
...
const QUICK_QTY = [1, 2, 3, 5, 10, 20, 50];

// ─── PDF ─────────────────────────────────────────────────────────────────────

function generateRestockPDF(snap: RestockSnapshot) {
  const fmt = (n: number) => n.toLocaleString('es-AR');
  const d = new Date(snap.date);
  const dateStr = d.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' });
  const timeStr = d.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' });

  const rows = snap.items.map(item => `
    <tr>
      <td>${item.name}${item.variantColor ? `<br><small>${item.variantColor}</small>` : ''}</td>
      <td class="center">${item.currentStock}</td>
      <td class="center bold green">+${item.qty}</td>
      <td class="center">${item.currentStock + item.qty}</td>
      <td class="right">${item.unitCost > 0 ? '$' + fmt(item.unitCost) : '—'}</td>
      <td class="right">${item.unitCost > 0 ? '$' + fmt(item.qty * item.unitCost) : '—'}</td>
    </tr>
  `).join('');
...
<head>
  <meta charset="UTF-8" />
  <title>Remito de Ingreso ${snap.orderNumber}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Segoe UI', Arial, sans-serif; font-size: 13px; color: #111; background: #fff; padding: 36px 40px; max-width: 720px; margin: 0 auto; }
    ...
    .order-num { font-size: 13px; color: #16a34a; font-weight: 700; margin-top: 2px; }
    ...
    .totals { border-top: 2px solid #111; }
    ...
    .notes { margin-top: 16px; padding: 10px 14px; background: #f9f9f9; border-left: 3px solid #16a34a; font-size: 12px; color: #444; }
...
<body>
  <div class="header">
    ...
    <div class="order-meta">
      <div class="order-title">Remito de Ingreso</div>
      <div class="order-num">${snap.orderNumber}</div>
      <div class="order-date">${dateStr} · ${timeStr}</div>
    </div>
  </div>

  <div class="info-row">
    ${snap.supplier ? `<div class="info-block"><label>Proveedor</label><strong>${snap.supplier}</strong></div>` : ''}
    <div class="info-block">
      <label>Artículos ingresados</label>
      <strong>${snap.items.reduce((s, i) => s + i.qty, 0)} unidades</strong>
    </div>
    ${snap.totalCost > 0 ? `<div class="info-block"><label>Costo total</label><strong>$${fmt(snap.totalCost)}</strong></div>` : ''}
  </div>
      ...
      <table class="totals-table">
        <tbody>
          <tr class="total-row">
            ...
            <td><strong>$${fmt(snap.totalCost)}</strong></td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  ${snap.notes ? `<div class="notes"><strong>Notas</strong>${snap.notes}</div>` : ''}

  <div class="footer">
    <strong>Remito de ingreso de stock — uso interno</strong><br>
    NicTech · ${dateStr}
  </div>
...
interface Props {
  open: boolean;
  onClose: () => void;
  onRestockComplete: (productId: string, newStock: number) => void;
  variantProductIds: Set<string>;
}

// ─── Component ────────────────────────────────────────────────────────────────

export function QuickRestockModal({ open, onClose, onRestockComplete, variantProductIds }: Props) {
  ...
  const [completedRestock, setCompletedRestock] = useState<RestockSnapshot | null>(null);
  ...
  const resetAll = () => {
    ...
    setQty('1'); setCost('');
    setCart([]); setSupplier(''); setNotes('');
    setCompletedRestock(null);
  };

  const clearSelection = () => {
    setSelected(null); setVariants([]); setSelVariant(null);
    setQty('1'); setCost(''); setQuery(''); setShowResults(false);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      const { data } = await (supabase as any)
        .from('products')
        .select('id, name, price, stock, image_url, category:categories(name)')
        .ilike('name', `%${query}%`)
        .order('name')
        .limit(10);
      if (data) {
        setResults(data.map((p: any) => ({
          ...
          image_url: p.image_url, category_name: p.category?.name ?? '',
  ...
  const currentStock = selected?.has_variants && selVariant
    ? selVariant.stock
    : (selected?.stock ?? 0);
  const canAdd = !!selected && (!selected.has_variants || !!selVariant) && qtyNum > 0;
    ...
    setCart(prev => {
      ...
      if (existing) {
        return prev.map(i => i.key === key
          ? { ...i, qty: i.qty + qtyNum, unitCost: costNum }
          : i);
      }
      return [...prev, {
        key,
  ...
  const cartTotalCost = cart.reduce((s, i) => s + i.qty * i.unitCost, 0);
  ...
  const confirmRestock = async () => {
    ...
    for (const item of cart) {
      try {
        if (item.variantId) {
          const newVariantStock = item.currentStock + item.qty;
          await (supabase as any)
            .from('product_variants')
            .update({ stock: newVariantStock })
            .eq('id', item.variantId);

          if (!recalced.has(item.productId)) {
            recalced.add(item.productId);
            const { data: allV } = await (supabase as any)
              .from('product_variants')
              .select('stock')
              .eq('product_id', item.productId);
            const total = (allV || []).reduce((s: number, v: any) => s + (v.stock || 0), 0);
            await (supabase as any)
              .from('products')
              .update({ stock: total })
              .eq('id', item.productId);
            onRestockComplete(item.productId, total);
          }
        } else {
    ...
    const now = new Date();
    const orderNumber = `#ING-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`;
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-3xl p-0 overflow-hidden gap-0 h-[90vh] flex flex-col">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-green-600 to-emerald-500 px-5 py-3 flex items-center flex-shrink-0">
          <DialogTitle className="text-white font-bold text-base flex items-center gap-2">
            <PackagePlus className="h-4 w-4" /> Reposición Rápida
          </DialogTitle>
        </div>
            ...
            <div className="flex-1 p-4 space-y-4">
              {!selected ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-3 py-12">
                  <PackagePlus className="h-10 w-10 opacity-20" />
                  <p className="text-sm">Buscá un producto para reponer stock</p>
                  <p className="text-xs opacity-60">El stock se suma al existente</p>
                </div>
              ) : (
                <>
                  {/* Selected product */}
                  <div className="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/30">
                    <ProductThumb url={selected.image_url} />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm leading-tight truncate">{selected.name}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {selected.category_name} ·{' '}
                        <span className={`font-medium ${selected.stock === 0 ? 'text-red-500' : ''}`}>
                          {selected.stock} en stock
                        </span>
                        {selected.has_variants && <span className="text-purple-600 ml-1">· seleccioná variante</span>}
                      </p>
                    </div>
                  ...
                  {(!selected.has_variants || selVariant) && (
                    <>
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between">
                            <Label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Cantidad a ingresar</Label>
                            {qtyNum > 0 && (
                              <span className="text-xs font-semibold text-green-600">
                                {currentStock} → {currentStock + qtyNum}
                              </span>
                            )}
                          </div>
                          <Input
                        ...
                        <div className="space-y-1.5">
                          <Label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                            Costo unitario <span className="normal-case font-normal text-muted-foreground">(opcional)</span>
                          </Label>
                          <div className="relative">
                          ...
                          {costNum > 0 && qtyNum > 0 && (
                            <p className="text-xs text-muted-foreground text-right">
                              Total: <span className="font-bold text-foreground">${(costNum * qtyNum).toLocaleString('es-AR')}</span>
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <Button
                        onClick={addToCart}
                        disabled={!canAdd}
                        className="w-full h-10 gap-2 font-semibold bg-green-600 hover:bg-green-700 text-white border-0 disabled:opacity-40">
                        <Plus className="h-4 w-4" />
                        Agregar a la reposición
                        {canAdd && qtyNum > 0 && (
                          <span className="ml-auto font-bold opacity-90">+{qtyNum} ud.</span>
                        )}
                      </Button>
                    </>
                  )}
          ...
          <div className="w-72 flex-shrink-0 flex flex-col bg-muted/10">
            ...
            <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-background">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold">Reposición</span>
                {cart.length > 0 && (
                  <span className="bg-green-600 text-white text-xs font-bold px-1.5 py-0.5 rounded-full leading-none">
                    {cart.length}
                  </span>
                )}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2 min-h-0">
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground gap-2 py-8">
                  ...
                  <p className="text-xs">Agregá productos para reponer</p>
                </div>
              ) : cart.map(item => (
                <div key={item.key} className="bg-background rounded-xl border border-border p-2.5 space-y-2">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.currentStock} → <span className="text-green-600 font-bold">{item.currentStock + item.qty}</span>
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 rounded-lg border border-border overflow-hidden">
                      ...
                      <span className="px-2 text-sm font-bold min-w-[1.5rem] text-center text-green-600">+{item.qty}</span>
                      <button type="button"
                        onClick={() => changeQty(item.key, 1)}
                    ...
                    <div className="text-right">
                      {item.unitCost > 0
                        ? <p className="text-sm font-bold">${(item.qty * item.unitCost).toLocaleString('es-AR')}</p>
            ...
            <div className="border-t border-border px-4 py-3 space-y-3 bg-background">
              ...
              {/* Summary */}
              {(cart.length > 0 || completedRestock) && (
                <div className="flex items-center justify-between rounded-xl px-3 py-2.5 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 border border-green-200 dark:border-green-800">
                  <div className="text-xs text-muted-foreground">
                    {(() => {
                      const units = completedRestock
                        ? completedRestock.items.reduce((s, i) => s + i.qty, 0)
                        : cartUnits;
                      const count = completedRestock ? completedRestock.items.length : cart.length;
                      return <>
                        <p className="text-green-600 font-semibold">+{units} unidades</p>
                  ...
                  <div className="text-right">
                    {(() => {
                      const total = completedRestock ? completedRestock.totalCost : cartTotalCost;
                      return total > 0
                        ? <><p className="text-xs text-muted-foreground">Costo</p><p className="text-lg font-bold">${total.toLocaleString('es-AR')}</p></>
                        : <p className="text-xs text-muted-foreground">sin costo registrado</p>;
                    })()}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1">
                <Label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Proveedor <span className="normal-case font-normal">(opcional)</span>
                </Label>
                <Input
              ...
              {completedRestock ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-2 rounded-xl px-3 py-2.5 bg-green-100 dark:bg-green-950/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold leading-tight">Stock actualizado</p>
                      <p className="text-xs opacity-70">{completedRestock.orderNumber}</p>
                    </div>
                  </div>
                  <Button
                    onClick={() => generateRestockPDF(completedRestock)}
                    className="w-full h-9 gap-2 font-semibold bg-blue-600 hover:bg-blue-700 text-white border-0">
                    <FileText className="h-4 w-4" /> Generar Remito
                  </Button>
                  <Button
                    variant="outline"
                    onClick={() => { setCompletedRestock(null); setTimeout(() => searchRef.current?.focus(), 80); }}
                    className="w-full h-9 gap-2 text-sm">
                    <RotateCcw className="h-3.5 w-3.5" /> Nueva reposición
                  </Button>
                </div>
              ) : (
                <Button
                  ...
                  className="w-full h-10 font-bold bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-700 hover:to-emerald-600 text-white border-0 gap-2 disabled:opacity-40">
                  {saving
                    ? <Loader2 className="h-4 w-4 animate-spin" />
                    : <TrendingUp className="h-4 w-4" />}
                  {saving ? 'Procesando…' : cart.length === 0 ? 'Lista vacía' : `Confirmar · ${cart.length} ítem${cart.length !== 1 ? 's' : ''}`}
                </Button>
              )}
