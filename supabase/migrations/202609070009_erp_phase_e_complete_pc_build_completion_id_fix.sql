-- Migration: 202609070009_erp_phase_e_complete_pc_build_completion_id_fix.sql
-- Description: Phase E fix 42702 residual en erp.complete_pc_build (variable vs columna).
-- La variable local completion_id colisiona con erp.pc_component_lineage.completion_id en el
-- IF de verificacion (l.completion_id=completion_id): el nombre pelado es ambiguo (42702).
-- Se renombra la variable a v_completion_id en DECLARE, RETURNING INTO, SELECT, IF y
-- complete_stage7_command. La columna completion_id del INSERT y l.completion_id quedan intactas.
...
begin
  ...
  insert into erp.pc_build_completions(organization_id,branch_id,project_id,revision_id,reservation_batch_id,stock_document_id,equipment_id,completed_by) values(org_id,project.branch_id,project.id,rev.id,target_reservation_batch_id,stock_id,equipment_id,auth.uid()) returning id into v_completion_id;
  ...
  select org_id,v_completion_id,c.id,c.product_id,c.variant_id,c.inventory_unit_id,c.quantity,
  ...
  if (select count(*) from erp.pc_component_lineage l where l.completion_id=v_completion_id)<>(select count(*) from erp.pc_build_components where revision_id=rev.id) then raise exception using errcode='check_violation',message='fulfillment cost lineage does not exactly match build components'; end if;
  insert into erp.pc_build_state_events(organization_id,branch_id,project_id,state,reason,actor_id) values(org_id,project.branch_id,project.id,'completed',operation_reason,auth.uid()); perform erp.complete_stage7_command(cmd.id,v_completion_id); return equipment_id;
end $$;
