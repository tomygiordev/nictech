-- Migration: 202609070007_erp_phase_e_complete_pc_build_fix.sql
-- Description: Phase E fix 42P01 en erp.complete_pc_build (referencias calificadas a funcion).
-- Linea 618 de 202608190008: complete_pc_build.warranty_snapshot pasa a warranty_snapshot (parametro).
-- Linea 629 de 202608190008: complete_pc_build.completion_id pasa a completion_id (variable local).
-- Postgres no resuelve nombre_de_funcion.columna dentro del cuerpo y lo trata como rango inexistente
-- (error 42P01); eso tumbaba los tests 27-29 de Fase E.
...
begin
  ...
  select org_id,completion_id,c.id,c.product_id,c.variant_id,c.inventory_unit_id,c.quantity,
    ...
    case when c.warranty_snapshot='{}' then warranty_snapshot else c.warranty_snapshot end
